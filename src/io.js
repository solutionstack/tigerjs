...
TigerJS.io = function(configObj) {
    ...
    var rval = function(configObj) {
        ...
        this.handleRequestErrorsInternally = function() {
            ...
            if (T.NETWORK_STATE === "OFFLINE") {
                if (configObj.subscribe.onerror)
                    configObj.subscribe.onerror.apply(this, ["NETWORK_ERROR", 0]);
            } else {
                //WE'RE ONLINE PROBABLY A DNS, PROXY OR GATEWAY ERROR
                if (configObj.subscribe.onerror)
                    configObj.subscribe.onerror.apply(this, ["NETWORK_ERROR", 1]);
...
TigerJS.io.CompositeRequest = function() {
    ...
    return new function() {
        ...
        var request_container = (T.Iterator()).addAll(arguments_), // hold references to all requests arguments to be sent
        ...
        request_container.foward_iterator(
            function(x) {
                ...
                globalConfigObj[request_index_counter + "_" + x.tag] += x.uniqueID === true ? Math.random().toString() : "";
