...
TigerJS.nodes = function(config, strictMatch) {
    return new(function(config_, strictMatch_) {
        ...
        if (pred && pred.childNodes) { //1st check for Child nodes of some element, if requested
            ...
            sib = T.$(pred['childNodes']).firstChild; //we are wrapping the index in T.$ in case it came in as a string ID
        ...
        try {
            ...
            if (pred && pred['_class']) {
                ...
                var cls = pred['_class'].trim().split(" "),
                ...
                for (i = 0; i < childNodes.length; i++) {
                    ...
                    if (childNodes[i].className && childNodes[i].attributes && childNodes[i].attributes.getNamedItem("class")) {
                        ...
                        arg = T.Iterator(childNodes[i].attributes.getNamedItem("class").nodeValue.trim().split(" ")); //get the classes on each element
                        //culd also use element.className in the above, guess am just flexin ma DOM muscle

                        if (arg.intersect(cls).length) { //if the two class array intersects, that is if the have a matching CSS class-name
    ...
    })(config, strictMatch).n; // return the NodeList-ish collection Object
};